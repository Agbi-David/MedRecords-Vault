...
const SearchFamily: React.FC<SearchFamilyProps> = ({ institutionId }) => {
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            const familyResponse = await fetch(`/api/families?where[familyCode][equals]=${familyCode}`);
            ...
            const documentsResponse = await fetch(`/api/documents?where[family][equals]=${fetchedFamily.id}`);
            ...
            {family && (
                <Card className="mb-8">
                    ...
                    <CardContent>
                        ...
                        <p><strong>Contact Email:</strong> {family.contactEmail}</p>
                                    ...
                                    <TableRow key={doc.id}>
                                        ...
                                        <TableCell>
                                            {requests[doc.id]?.status === 'approved' ? (
                                                ...
                                                <Button onClick={() => handleRequestDocument(doc)}>Request</Button>
                                            )}
                                        </TableCell>
                                    </TableRow>
                                ))}
